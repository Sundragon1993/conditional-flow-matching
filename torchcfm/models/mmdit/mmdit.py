...
class DiT_Llama(nn.Module):
    ...
    def forward(self, x, y, t):
        self.freqs_cis = self.freqs_cis.to(x.device)
        x = self.init_conv_seq(x)  # 128,128,28,28

        x = self.patchify(x)  # 128,196,512
        x = self.x_embedder(x)  # 128,196,512
...
def DiT_Llama_600M_patch2(**kwargs):
    return DiT_Llama(patch_size=2, in_channels=1, dim=256, n_layers=16, n_heads=32, **kwargs)
